...
test('ProteinBank renders table controls, bulk actions, and row action triggers', () => {
    ...
    const markup = renderToStaticMarkup(_jsx(ProteinBank, { rows: rows, selectedId: sampleProteins[0].id, filters: defaultProteinBankFilters(), sortKey: "title", pinnedIds: new Set(), onFiltersChange: () => undefined, onSortKeyChange: () => undefined, onSearchResults: () => undefined, onOpenProtein: () => undefined, onTogglePinned: () => undefined }));
