...
test('SequenceViewer renders selected residue state in the sequence track', () => {
    ...
    const markup = renderToStaticMarkup(_jsx(SequenceViewer, { protein: protein, chainFilter: "all", structureLevel: "primary", sequenceState: {
            activeChainId: 'A',
            density: 'residues',
        ...
        }, onDensityChange: () => undefined, onChainFocusChange: () => undefined }));
...
test('ExplorerSidepanel general tab includes the metadata section safely with partial metadata', () => {
    const protein = sampleProteins[0];
    const markup = renderToStaticMarkup(_jsx(ExplorerSidepanel, { protein: protein, chainFilter: "all", activeTab: "general", structureLevel: "secondary", sequenceState: { activeChainId: 'all', density: 'overview', focusedResidueId: null, highlightedRange: null }, onTabChange: () => undefined, onStructureLevelChange: () => undefined, onSequenceStateChange: () => undefined, onTargetSelect: () => undefined }));
...
test('ExplorerSidepanel structure tab renders structure controls', () => {
    const protein = sampleProteins[0];
    const markup = renderToStaticMarkup(_jsx(ExplorerSidepanel, { protein: protein, chainFilter: "all", activeTab: "structure", structureLevel: "secondary", sequenceState: { activeChainId: 'all', density: 'overview', focusedResidueId: null, highlightedRange: null }, onTabChange: () => undefined, onStructureLevelChange: () => undefined, onSequenceStateChange: () => undefined, onTargetSelect: () => undefined }));
...
test('ExplorerSidepanel sequence tab renders the new track and residue detail panel', () => {
    const protein = sampleProteins[0];
    const markup = renderToStaticMarkup(_jsx(ExplorerSidepanel, { protein: protein, chainFilter: "all", activeTab: "sequence", structureLevel: "secondary", sequenceState: { activeChainId: 'A', density: 'overview', focusedResidueId: protein.chains[0].residues[0].id, highlightedRange: null }, onTabChange: () => undefined, onStructureLevelChange: () => undefined, onSequenceStateChange: () => undefined, onTargetSelect: () => undefined, selectedResidue: {
...
test('ProteinViewer renders scale HUD and orientation widget affordances', () => {
    const markup = renderToStaticMarkup(_jsx(ProteinViewer, { protein: sampleProteins[0], chainFilter: "all", viewMode: "backbone", showAxes: false, sceneSettings: { unitSystem: 'angstrom', showGrid: true, showFog: true, lightingPreset: 'scientific' }, fitViewNonce: 0, onSceneSettingsChange: () => undefined, structureLevel: "secondary" }));
