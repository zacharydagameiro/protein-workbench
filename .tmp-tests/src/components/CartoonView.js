...
const colorForStructure = (structure) => {
    switch (structure) {
        case 'helix':
...
function RibbonSegment({ atoms, color, radius }) {
    ...
    return (_jsx("mesh", { geometry: geometry, children: _jsx("meshStandardMaterial", { color: color }) }));
}
export default function CartoonView({ protein, visibleChainIds }) {
    const residueIndex = useMemo(() => {
        const entries = protein.chains.flatMap((chain) => chain.residues.map((residue) => [residue.id, residue.secondaryStructure]));
    ...
    const segments = useMemo(() => {
        ...
        for (const [chainId, atoms] of byChain.entries()) {
            ...
            let currentSegmentAtoms = [];
            let currentStructure = 'loop';
            for (const atom of sortedAtoms) {
                const structure = residueIndex.get(`${chainId}:${atom.residueNumber ?? 0}:${atom.insertionCode ?? ''}`) ?? 'loop';
    ...
    return (_jsx(_Fragment, { children: segments.map((segment) => (_jsx(RibbonSegment, { ...segment }, segment.id))) }));
}
