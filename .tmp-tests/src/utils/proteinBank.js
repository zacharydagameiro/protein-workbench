const defaultRowText = (value, fallback = 'Unavailable') => value?.trim() || fallback;
export const buildProteinBankRows = ({ starterProteins, pinnedProteins, historyProteins, searchResults, }) => {
    const rows = new Map();
    const register = (protein, collection) => {
...
export const defaultProteinBankFilters = () => ({
    collection: 'all',
    source: 'all',
    organism: 'all',
...
export const getProteinBankOptions = (rows) => ({
    organisms: [...new Set(rows.map((row) => row.organism).filter((value) => value !== 'Unavailable'))].sort((left, right) => left.localeCompare(right)),
    methods: [...new Set(rows.map((row) => row.experimentalMethod).filter((value) => value !== 'Unavailable'))].sort((left, right) => left.localeCompare(right)),
});
export const filterProteinBankRows = (rows, filters) => rows.filter((row) => {
...
export const sortProteinBankRows = (rows, sortKey) => {
    const sorted = [...rows];
