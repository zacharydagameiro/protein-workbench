...
const atomElementForIndex = (index) => {
    if (index % 7 === 0) {
        return 'N';
    }
    if (index % 5 === 0) {
...
const buildResidue = (proteinId, chainId, residueNumber, sequenceIndex, x, y, z, secondaryStructure) => {
...
const buildChain = (proteinId, chainId, length, positionForResidue, helixRange) => {
    const residues = [];
    const atoms = [];
    const backboneAtoms = [];
    const helices = [];
    for (let index = 0; index < length; index += 1) {
        ...
        const secondaryStructure = helixRange && residueNumber >= helixRange[0] && residueNumber <= helixRange[1]
            ? 'helix'
            : 'loop';
        const [x, y, z] = positionForResidue(index);
        const built = buildResidue(proteinId, chainId, residueNumber, index, x, y, z, secondaryStructure);
...
const buildSampleProtein = (id, name, description, chainBuilders, variants = [], sheets = []) => {
...
export const sampleProteins = [
    buildSampleProtein('sample-helix', 'Signal Helix Starter', 'A compact alpha-helical starter model with a single structured chain.', [
        () => buildChain('sample-helix', 'A', 18, (index) => {
            const t = index * 0.55;
    ...
    buildSampleProtein('sample-dimer', 'Regulatory Dimer Starter', 'Two curated chains that make comparison and chain filtering useful without requiring a network fetch.', [
        () => buildChain('sample-dimer', 'A', 14, (index) => {
            const angle = index * 0.35;
            return [Math.cos(angle) * 2.7 - 3.2, Math.sin(index * 0.4) * 0.8, Math.sin(angle) * 1.9];
        }, [4, 10]),
        () => buildChain('sample-dimer', 'B', 12, (index) => {
