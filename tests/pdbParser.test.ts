...
const buildHelixLine = (chainId: string, startResidue: number, endResidue: number) =>
  `HELIX    1   1 ALA ${chainId}${startResidue.toString().padStart(4, ' ')}  LYS ${chainId}${endResidue
    .toString()
    .padStart(4, ' ')}  1                                  ${String(endResidue - startResidue + 1).padStart(2, ' ')}`;

const buildSheetLine = (chainId: string, startResidue: number, endResidue: number) =>
  `SHEET    1   A 1 THR ${chainId}${startResidue.toString().padStart(4, ' ')}  TYR ${chainId}${endResidue
    .toString()
    .padStart(4, ' ')}  0`;
