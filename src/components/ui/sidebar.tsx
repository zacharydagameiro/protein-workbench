  ...
  return (
    <>
      ...
      <aside
        ref={ref}
        className={cn(
          'fixed inset-y-0 z-50 flex w-[20rem] flex-col border-r border-sidebar-border/70 bg-sidebar text-sidebar-foreground shadow-2xl transition-transform duration-200 lg:static lg:translate-x-0 lg:shadow-none',
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn('flex h-screen min-h-0 min-w-0 flex-1 flex-col overflow-hidden', className)} {...props} />
));
...
function SidebarTrigger({ className, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button variant="outline" size="icon" className={className} onClick={() => setOpen((current) => !current)} {...props}>
...
const SidebarHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn('flex flex-col gap-3 p-3', className)} {...props} />
));
SidebarHeader.displayName = 'SidebarHeader';

const SidebarContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn('flex min-h-0 flex-1 flex-col gap-4 overflow-hidden p-3 pt-0', className)} {...props} />
));
SidebarContent.displayName = 'SidebarContent';

const SidebarFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn('mt-auto border-t border-sidebar-border/70 p-3', className)} {...props} />
));
...
const SidebarGroupLabel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn('px-2 text-xs font-medium tracking-[0.08em] text-sidebar-foreground/55', className)} {...props} />
));
...
const SidebarMenuItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn('group/menu-item relative', className)} {...props} />
));
...
>(({ className, isActive, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn(
        'flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left text-sm transition-colors outline-none',
        isActive
          ? 'bg-sidebar-accent text-sidebar-accent-foreground'
          : 'text-sidebar-foreground/78 hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground',
        className,
      )}
      {...props}
    />
...
const SidebarMenuAction = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(({ className, ...props }, ref) => (
  <button
    ref={ref}
    className={cn(
      'absolute right-1 top-1/2 -translate-y-1/2 rounded-md p-1 text-sidebar-foreground/55 opacity-0 transition-opacity group-hover/menu-item:opacity-100 group-focus-within/menu-item:opacity-100 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground',
      className,
    )}
    {...props}
  />
...
function SidebarRail({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('w-px bg-sidebar-border/40', className)} {...props} />;
}
