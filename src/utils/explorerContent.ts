...
const formatWord = (word: string, isFirstWord: boolean): string => {
...
export const normalizeDisplayTitle = (rawTitle: string, fallback = 'Unknown structure'): string => {
...
export const getTeachingCodonEntry = (aminoAcid: string): TeachingCodonEntry => {
...
const residueIdFromParts = (chainId: string, residueNumber: number, insertionCode?: string): string =>
  `${chainId}:${residueNumber}:${insertionCode ?? ''}`;

const buildLoopRegions = (chain: ChainData, occupied: Array<[number, number]>): RegionAnnotation[] => {
  const sortedIntervals = [...occupied].sort((left, right) => left[0] - right[0]);
  const loops: RegionAnnotation[] = [];
...
export const buildDerivedRegions = (protein: Protein): RegionAnnotation[] => {
  const chainRegions: RegionAnnotation[] = protein.chains.map((chain) => ({
  ...
  const helixRegions: RegionAnnotation[] = protein.helices.map((helix) => ({
    ...
    structureLevel: 'secondary' as const,
  }));

  const sheetRegions: RegionAnnotation[] = protein.sheets.flatMap((sheet) =>
    sheet.strands.map((strand, index) => ({
  ...
  const loops = protein.chains.flatMap((chain) => {
    const occupied = [
      ...helixRegions.filter((region) => region.chainId === chain.id).map((region) => [region.startResidue, region.endResidue] as [number, number]),
      ...sheetRegions.filter((region) => region.chainId === chain.id).map((region) => [region.startResidue, region.endResidue] as [number, number]),
    ];
...
const pickFocusResidue = (protein: Protein): ViewerSelection | null => {
...
const summarizeStructureLevel = (protein: Protein): { level: StructureLevel; label: string; body: string } => {
...
export const buildStoryCards = (protein: Protein): StoryCard[] => {
  const focusResidue = pickFocusResidue(protein);
  const focusRegion =
    protein.regions.find((region) => region.kind === 'helix' || region.kind === 'sheet' || region.kind === 'domain') ??
...
export const buildRecentItems = (proteins: Protein[]): RecentItem[] =>
  proteins.slice(0, 8).map((protein) => ({
    id: protein.id,
...
export const getStructureLevelDescription = (protein: Protein, level: StructureLevel): { title: string; body: string } => {
  switch (level) {
    case 'primary':
      return {
...
export const getTargetedResidue = (target: ViewerTarget | null): ViewerSelection | null => {
  if (!target) {
    return null;
  }
...
export const buildComparisonSummary = (proteins: Protein[]): string => {
...
export const enrichProteinForExplorer = (protein: Protein): Protein => {
  ...
  const regions = [...withMetadata.regions, ...buildDerivedRegions(withMetadata)].filter(
    (region, index, allRegions) => allRegions.findIndex((candidate) => candidate.id === region.id) === index,
  );
  const withRegions = {
    ...withMetadata,
    regions,
