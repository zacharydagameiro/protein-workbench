...
const parseElement = (atomName: string, elementColumn: string): AtomElement => {
...
export const parsePDBAtoms = (pdbContent: string): ParsedAtom[] => {
...
export const parseSecondaryStructure = (
  pdbContent: string,
): {
  helices: Helix[];
  sheets: Sheet[];
  ...
  const helices: Helix[] = [];
  const sheetMap = new Map<number, Sheet>();
...
export const parseSEQRES = (pdbContent: string): Map<string, string> => {
...
const buildChains = (
  ...
  const residuesByChain = new Map<string, Map<string, ResidueData>>();

  for (const atom of atoms) {
    const chainResidues = residuesByChain.get(atom.chainId) ?? new Map<string, ResidueData>();
...
const extractBackboneAtoms = (atoms: ParsedAtom[]): Atom[] =>
  atoms.filter((atom) => atom.atomName === 'CA');
...
export const pdbToProtein = (pdbContent: string, pdbId: string, name?: string): Protein => {
